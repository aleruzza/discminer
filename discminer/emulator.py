...
class BaseEmulator():
    ...
    def __init__(self, model_pth='', model_para={}, device='cpu'):
        self.params = model_para
        self.device = device
        self.emulator = create_nnmodel(n_param=self.params['n_param'], image_size=self.params['image_size'], num_channels=self.params['num_channels'],
                                        num_res_blocks=self.params['num_res_blocks'], channel_mult=self.params['channel_mult'],
                                        mode=self.params['mode'], unc=self.params['unc']).to(device=torch.device(self.device))
...
class Emulator():
    ...
    def __init__(self, model_pths=[], model_params=[], device='cpu', labels=['dens', 'vphi', 'vr'], ict_gen=generate_ict_128x128_disc_tri):
        self.device = device
        self.params = []
        self.emulators = []
        ...
        for i in range(len(model_params)):
            ...
            self.emulators.append(BaseEmulator(model_pths[i], self.params[i], device = self.device))
    
    def emulate(self, alpha, h, planetMass, sigmaSlope, flaringIndex):
        ic = self.ict_gen(slopes=np.array([sigmaSlope]), dimension=self.params[0]['image_size'])
        ...
        for i, emu in enumerate(self.emulators):
            result.append(emu.emulate(ic[:,[i]], norm_params).detach())
    ...
    def emulate_v3d(self, coord, alpha, h, planetMass, sigmaSlope, flaringIndex, R_p, phi_p, extrap_vfunc, **extrap_kwargs):
        ...
        v3d = self.emulate(alpha, h, planetMass, sigmaSlope, flaringIndex)[0,1:].detach().numpy()
        v3d = v3d[:,:,::-1] 
        ...
        if 'phi' not in coord.keys(): phi = np.arctan2(coord['x'], coord['y'])
        else: phi = coord['phi'] 
        
        vphi_interp = (griddata((x_dom, y_dom), v3d_dom[0].reshape(-1), (coord['x'], coord['y']))*np.sqrt(G*Mstar*u.MSun/R_p)) *1e-3*(-1)*v_sign
        vr_interp = griddata((x_dom, y_dom), v3d_dom[1].reshape(-1), (coord['x'], coord['y']))*1e-3
        
        print(R)
        mask = (R>2.8*R_p) | (R<0.5*R_p)
        vphi_interp[mask] = extrap_vfunc(coord, **extrap_kwargs)[mask]
        vr_interp[mask] = 0
        v3d_interp = np.concatenate([np.expand_dims(vphi_interp, axis=0), np.expand_dims(vr_interp, axis=0), -np.zeros((1,*vphi_interp.shape))*v_sign], axis=0)
